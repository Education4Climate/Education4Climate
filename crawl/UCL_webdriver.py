...
# mapping : [prerequisite,theme,goal,content,method,evaluation other, resources,
# biblio,faculty,anacs,shortname,class,location,teachers,language]
def get_program_infos(href, driver):
    ...
    hrefs_elems = [e.get_attribute("href") for e in
                   driver.find_elements_by_xpath("//a[contains(@href,'{}')]".format(programme_code.lower()))]
    ...
    if sum([h.endswith("-ppm") for h in hrefs_elems]) > 0:
        ...
        courses_hrefs = [e.get_attribute("href") for e in
                         driver.find_elements_by_xpath("//a[contains(@href,'cours-2020')]")]
...
def get_course_page_infos(href, driver):
    ...
    for k, v in zip(s1, s2):
        key = k.text
        value = v.text
        if "enseignant" in key.lower():
            infos["teachers"] = value.split(";")
        elif "langue" in key.lower():
            infos["language"] = value.lower()
        elif "contenu" in key.lower():
            infos["content"] = value
        elif "thèmes" in key.lower():
            infos["theme"] = value
        elif "acquis" in key.lower():
            infos["goal"] = value
        elif "méthode" in key.lower():
            infos["method"] = value
        elif "faculté" in key.lower():
            infos["faculty"] = value
        elif "biblio" in key.lower():
            infos["bibliography"] = value
        elif "ressources" in key.lower():
            infos["resources"] = value
    return infos
...
if __name__ == "__main__":
    ...
    # TODO: this is not working is it?
    print("already crawled : {} programs, {} courses\n ".format(len(programs.keys()), len(courses.keys())))

    # Get faculties
    ucl_driver.driver.get(url_ucl)
    faculties = {e.text: e.get_attribute("href") for e in
                 ucl_driver.driver.find_elements_by_xpath("//a[contains(@class,'first')]")}
    ...
    programs = {k: v for k, v in programs.items() if
                "bachelier" in v["name"].lower() or "master" in v["name"].lower() or "mineur" in v["name"].lower()}
