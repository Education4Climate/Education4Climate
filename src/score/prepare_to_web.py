...
def main():
    scores = pd.read_csv('data/scoring-output/ucl_scoring_2020.csv')
    scores = scores.drop(columns=["class", "teachers"]).rename(columns={"code": "id"})
    js = json.load(open("data/crawling-output/ucl_courses_2020.json"))
    ...
    join.to_csv('data/web-ui-input/ucl_data_2020_heavy.csv')
    join[join["shift_score"] != 0].to_csv('data/web-ui-input/ucl_data_2020_light.csv')
