...
def get_shift_patterns(languages):
    ...
    for language in languages:
        ...
        pattern_shift = {u.refactor_pattern(pat): weight for pat, weight in zip(df_pattern_shift[columns[0]].values.tolist(), df_pattern_shift[columns[-1]].values.tolist())}
...
def main(args):
    ...
    for i, row in tqdm(df_courses.iterrows(), total=df_courses.shape[0]):  # TODO: voir si df_courses.apply() pourrait s'appliquer ici
        ...
        for odd, b in odd_scores.items(): data[odd] = int(b) #TODO Replace by literal statement
