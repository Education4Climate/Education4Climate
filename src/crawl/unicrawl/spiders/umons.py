...
class UmonsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        base_urls = [
            ...
            "https://web.umons.ac.be/fmp/fr/loffre-de-formation-2/"
        ...
        program_urls = ["https://web.umons.ac.be/droit/fr/training-offer/ba-bdroit/",
    ...
    def parse_faculty(self, response):
        ...
        # masters_string_1 = "Second cycle : Master’s degree"
        masters_string= "2ème Cycle: Master"

        # Leaving out PhDs, formations for adults and other special formations.
        bachelors = response.xpath(f'//li[a/text()="{bachelor_string}"]/ul/li/a/@href').getall()
        # if not bachelors:
        #     bachelors = response.xpath(f'//li[a/text()="{bachelor_string_2}"]/ul/li/a/@href').getall()
        
        masters = response.xpath(f'//li[a/text()="{masters_string}"]/ul/li/a/@href').getall()
        # if not masters:
        #     masters = response.xpath(f'//li[a/text()="{masters_string_2}"]/ul/li/a/@href').getall()

        if not bachelors:
            bachelors_details = response.xpath(f'//a[text()="{bachelor_string}"]/@href').get()
            # if not bachelors_details:
            #     bachelors_details = response.xpath(f'//a[text()="{bachelor_string_2}"]/@href').get()
        ...
        if not masters:
            masters_details = response.xpath(f'//a[text()="{masters_string}"]/@href').get()
            # if not masters_details:
            #     masters_details = response.xpath(f'//a[text()="{masters_string_2}"]/@href').get()
    ...
    def parse_details(self, response):
        programs = response.xpath('//article[starts-with(@class,"shortcode-training training-small scheme-")]/a/@href').getall()
    ...
    def parse_prog(self, response):
        ...
        if not href:
            ...
            location = response.xpath('//div[div/text()="Lieu"]').css('div.value::text').get()
    ...
    def parse_prog_detail(self, response, location):
        faculty = response.css('td.facTitle::text')
