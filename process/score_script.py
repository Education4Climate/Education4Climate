...
def tokenize(text):
    return [unidecode(w.text.lower()) for w in nlp(text) if
            w.pos_ not in ["PUNCT", "NUM", "SYM", "SPACE"] and w.is_stop is False]
...
def get_pattern_shift(languages):
    ...
    for language in languages:
        ...
        pattern_shift = {refactor_pattern(pat): weight for pat, weight in
...
if __name__ == "__main__":
    ...
    for i, row in df_courses.iterrows():
        ...
        data = {"code": row.shortname, "shift_score": score_shift, "shiftpatterns": json.dumps(match_shift),
                "class": row["class"], "teachers": row.teachers}
